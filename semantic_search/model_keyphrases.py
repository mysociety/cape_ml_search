...
def make_embeddings(split_corpus):
    ...
    my_vectorizer = KeyphraseCountVectorizer(pos_pattern= '<J.*>?<N.*>{1,2}') # look for candidates, 0 or 1 adjectives followed by 1 or 2 nouns
    ...
    with open('../data/raw/word_embeddings.pkl', "wb") as fOut:
        pickle.dump(
    ...
    #doc embeddings
    with open('../data/raw/split_plan_embeddings.pkl', "wb") as fOut:
...
def run_keybert_extraction(doc_embeddings,word_embeddings)    
    #define how diverse and how many keyphrases per plan                                                       )
    diversity = 0.6
    top_n=10
    
    #extract using keybert model. Output
    keyphrases = kw_model.extract_keywords(split_corpus,
                                    use_maxsum=False,
                                    use_mmr=True,
                                    diversity = diversity,
                                    top_n= top_n,
    ...
    with open('../data/raw/keyphrases-cosine_sim.csv', 'w') as f:
        writer = csv.writer(f)
        for row in keywords:
            writer.writerow(row)
