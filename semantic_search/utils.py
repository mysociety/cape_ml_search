...
def clean3(test: str):
    ...
    replacements = {'t-co2e': 'tonnes co2 emissions', 
    ...
    test = re.sub('[#*;•♦£$\[\]]', '.',test)
    test = re.sub('[-/]', ' ',test)
    test = re.sub('\.\s', '.', test) #think this causing problems
    test = re.sub('\.\.+', '.',test)
    test = re.sub("\([^)]+\)",'',test)
    ...
    test = re.sub('((?:\d{1,3}(?:,\d{3})+|\d+)(?:\.\d+)?)(m|bn|,|\.|\s|\b)', '',test) #getting rid of numbers
    test = ' '.join(test.split())
    test = re.sub('\.\.+', '.',test)
    ...
    test = ' '.join(test.split())
    # test = re.sub('\b\w{1,2}\b','',test) #removing 1 and 2 letter words
    #spell checker takes 20s per doc
    # blob = TextBlob(test)
    # test = str(blob.correct())
    return test
