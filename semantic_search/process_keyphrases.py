from utils import (
    has_repeated_word,
    contains_digits,
    contains_misspelled_word,
    word_a_b_common_word,
)
...
def get_unique_vocab():
    ...
    df = pd.read_csv(
        r"data\raw\keyphrases_cosine_sim.csv", header=None
    )  # entry in this csv formatted like e.g ('keyphrase', 0.8102)
    keywords = df.applymap(
        lambda x: x.split(",")[0][2:-1] if pd.notna(x) else x
    )  # only take keyphrase, not the cosine similarity to plan
    unique_keyphrases = list(set(keywords.values.ravel().tolist()))
    unique_keyphrases = unique_keyphrases[1:]  # 0th element is nan
...
def create_stats_csv(unique_keyphrases, plan_index):
    ...
    df = pd.read_csv(r"data\raw\keyphrases_cosine_sim.csv", header=None)
    keywords = df.applymap(lambda x: x.split(",")[0][2:-1] if pd.notna(x) else x)
    keywords["plan#"] = plan_index

    # create column for how many plans the keyphrase is a keyphrase in
    key_stats = pd.DataFrame({"keyphrase": unique_keyphrases})
    key_stats["keyphrase in _ plans"] = [
        len(keywords[(keywords == x).any(axis=1)]["plan#"].value_counts())
        for x in unique_keyphrases
    ]
    ...
    key_stats["has common word"] = key_stats["keyphrase"].apply(
        has_repeated_word
    )  # flags keyphrases like reducing reduction
    key_stats["contains digits"] = key_stats["keyphrase"].apply(contains_digits)
    key_stats["misspelled"] = [
        contains_misspelled_word(text) for text in key_stats["keyphrase"]
    ]

    # filter for useful keyphrases and save csv
    clean_key_stats = key_stats[
        (key_stats["has common word"] == False)
        & (key_stats["contains digits"] == False)
        & (key_stats["misspelled"] == False)
    ][
        ["keyphrase", "keyphrase in _ plans"]
    ]  #
    clean_key_stats.to_csv(r"data\final\ml_keyphrases.csv", index=False)
...
def create_pairwise_csv():
    ...
    df = pd.read_csv(r"data\final\ml_keyphrases.csv")
    ...
    with open(r"data\raw\word_embeddings.pkl", "rb") as fIn:
    ...
    df1_melt = df1.melt(
        ...
        var_name="nth similar",
        value_name="word_b",
    )
    df2_melt = df2.melt(
        id_vars=[0],
        value_vars=df2.columns[1:],
        var_name="nth similar",
    ...
    df_final.to_csv(r"data\final\ml_keyphrases_pairwise.csv", index=False)
